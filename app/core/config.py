class Config:
    ...
    llm_prompt = '這是只關注澳門地方消息的澳門電臺澳門講場的語音識別內容，我只要分析這段內容，給我以上內容分析後的標題，以上內容分析厚的總結（總結要列點說明，並且字數要 1000字以上，每一點中要有對該點字數不少於 300字 的單點分析），和主題點分析（要有5點以上，要有标题和 70到100字之間的句子段落的解释）。總結和主題點分析出來的內容不要有太多專業術語，因爲用戶是普通市民。並且分析出來的內容來源最好主要依賴上傳的文本。並且結合聯網功能搜索出來的內容比例最多 30% 。全部內容文字中不要出現搜索來源等的提示。最後分析出來的內容要給我結構化輸出：{title: "", summarization: [{title：""， content: ""}], subjects: [{title: "", content: ""}]}'
